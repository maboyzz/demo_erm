...
public class ClassifyReasonServiceImpl implements ClassifyReasonService {
    ...
    @Override
    public Long handleCreateClassifyReason(ClassifyReasonDTO dto) throws NameExisted {
    ...
    public ClassifyReasonDTO handleGetClassifyReasonById(Long id) {
        ...
        ClassifyReasonEntity classifyReason = this.classifyReasonRepository.findById(id).orElseThrow(() -> new BadRequestValidationException("Thẻ bảo hiểm với ID " + id + " không tồn tại"));
    ...
    @Override
    public Long handleUpdateClassifyReason(ClassifyReasonDTO dto) throws NameExisted {
        if (this.nameExists(dto.getName())) {
            throw new NameExisted("Name đã có");
        }
    ...
    @Override
    public ResultPaginationDTO<ClassifyReasonDTO> handleGetClassifyReason(String code, String name, List<Long> systemIds, Pageable pageable) {

        Specification<ClassifyReasonEntity> spec = Specification.where(ClassifyReasonSpecification.hasCode(code)).and(ClassifyReasonSpecification.hasName(name)).and(ClassifyReasonSpecification.hasSystemIdIn(systemIds));
